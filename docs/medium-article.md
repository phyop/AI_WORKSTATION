...
因此第一個決定不是開 Port 22，而是把所有工作納入一個 private Git repository，以 Git 作為非敏感設定與文件的唯一真實來源。Repository 建立了 README、架構、安全基線、Roadmap、威脅模型、八份 ADR、平台分離的 bootstrap 目錄，以及可自動驗證 Secret 測試檔確實被忽略的 PowerShell 腳本。
...
Meta description：從 private Git repository、最小權限帳號到 OpenSSH DISM/CBS 診斷，完整記錄如何以人工批准與可稽核腳本建立 Windows AI 工程工作站。
